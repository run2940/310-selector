...
function renderGroups(strategyId, label, overlaps) {
  const groups = new Map();
  toRows(payloadFor(strategyId)).forEach(row => {
    const key = row['細產業'] || row['產業'] || '未分類';
    const code = String(row['代碼']);
    if (!groups.has(key)) groups.set(key, new Map());
    groups.get(key).set(code, row);
  });
  const ranked = [...groups.entries()].map(([name, stocks]) => {
    const list = [...stocks.values()];
    const rs = list.map(row => number(row['歐奈爾RS評分(1-99)'])).filter(value => value !== null);
    const change = list.map(row => number(row['漲幅%'])).filter(value => value !== null);
    return {
      name,
      list,
      rs90: rs.filter(value => value >= 90).length,
      avgRs: rs.length ? rs.reduce((a, b) => a + b, 0) / rs.length : null,
      avgChange: change.length ? change.reduce((a, b) => a + b, 0) / change.length : null,
    };
  }).sort((a, b) => (b.rs90 - a.rs90) || ((b.avgRs ?? -Infinity) - (a.avgRs ?? -Infinity)) || ((b.avgChange ?? -Infinity) - (a.avgChange ?? -Infinity)));
  const strongestGroups = ranked.slice(0, 15);
  const items = state.search
