...
function visibleColumns(payload, rows) {
  const available = new Set(payload.columns); const selected = baseColumns.filter(column => available.has(column));
  orderedColumnGroups.forEach(group => { const column = group.find(item => available.has(item) && rows.some(row => row[item] !== null && row[item] !== undefined && row[item] !== '')); if (column) selected.push(column); });
...
function renderStrategyContent() {
  ...
  const header = columns.map(column => `<th><button data-sort="${escapeHtml(column)}">${escapeHtml(labels[column] || column)}${state.sort?.[0] === column ? (state.sort[1] === 1 ? ' ↑' : ' ↓') : ''}</button></th>`).join('');
  const body = rows.map(row => `<tr>${columns.map(column => { const raw = row[column]; const isOverlap = overlaps.has(String(row['代碼'])); if (column === '商品') { const code = escapeHtml(row['代碼']); return `<td><a class="${isOverlap ? 'overlap' : ''}" target="_blank" rel="noopener" href="https://fubon-ebrokerdj.fbs.com.tw/z/zc/zco/zco_${code}.djhtm">${isOverlap ? '◆ ' : ''}${escapeHtml(raw)}</a></td>`; } if (column === '代碼') return `<td class="${isOverlap ? 'overlap' : ''}">${isOverlap ? '◆ ' : ''}${escapeHtml(raw)}</td>`; const numeric = number(raw); const className = concentration.has(column) && numeric !== null ? (numeric > 0 ? 'positive' : numeric < 0 ? 'negative' : '') : ''; return `<td class="${className}">${escapeHtml(displayValue(column, raw))}</td>`; }).join('')}</tr>`).join('');
  root.innerHTML = `${note}<div class="section-heading"><span>${escapeHtml(strategy.label)} 選股清單</span></div><div class="table-shell selection-table"><table class="stock-table"><thead><tr>${header}</tr></thead><tbody>${body || `<tr><td colspan="${columns.length}">沒有符合搜尋條件的股票。</td></tr>`}</tbody></table></div><a class="download-link" href="data/${activeDay().strategies.find(item => item.id === strategy.id).file}" download>下載 ${escapeHtml(strategy.label)} JSON</a>${['rs-weighted','main-buy-up'].includes(strategy.id) ? renderGroups(strategy.id, strategy.label, overlaps) : ''}`;
