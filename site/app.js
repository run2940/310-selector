...
function renderStrategyContent() {
  ...
  root.innerHTML = `${note}<div class="section-heading list-heading"><span>${escapeHtml(strategy.label)} 選股清單</span></div><div class="table-shell selection-table"><table class="stock-table"><thead><tr>${header}</tr></thead><tbody>${body || `<tr><td colspan="${columns.length}">沒有符合搜尋條件的股票。</td></tr>`}</tbody></table></div>${['rs-weighted','main-buy-up'].includes(strategy.id) ? renderGroups(strategy.id, strategy.label, overlaps) : ''}`;
...
function renderGroups(strategyId, label, overlaps) { const groups = new Map(); toRows(payloadFor(strategyId)).forEach(row => { const key = row['細產業'] || row['產業'] || '未分類'; const code = String(row['代碼']); if (!groups.has(key)) groups.set(key, new Map()); groups.get(key).set(code, row); }); const ranked = [...groups.entries()].map(([name, stocks]) => { const list = [...stocks.values()]; const rs = list.map(row => number(row['歐奈爾RS評分(1-99)'])).filter(value => value !== null); const change = list.map(row => number(row['漲幅%'])).filter(value => value !== null); return { name, list, rs90:rs.filter(value => value >= 90).length, avgRs:rs.length ? rs.reduce((a,b)=>a+b,0)/rs.length : null, avgChange:change.length ? change.reduce((a,b)=>a+b,0)/change.length : null }; }).sort((a,b) => (b.rs90-a.rs90) || ((b.avgRs ?? -Infinity)-(a.avgRs ?? -Infinity)) || ((b.avgChange ?? -Infinity)-(a.avgChange ?? -Infinity))).slice(0,15); const items = state.search ? ranked.map(item => ({ ...item, list: item.list.filter(row => matchRow(row, state.search)) })).filter(item => item.list.length) : ranked; const showRsMetrics = strategyId === 'rs-weighted'; const body = items.map(item => `<tr><td>${escapeHtml(item.name)}</td><td>${item.list.length}</td><td>${item.list.map(row => `<span class="${overlaps.has(String(row['代碼'])) ? 'overlap' : ''}">${overlaps.has(String(row['代碼'])) ? '◆ ' : ''}${escapeHtml(row['代碼'])} ${escapeHtml(row['商品'])}</span>`).join('、')}</td>${showRsMetrics ? `<td>${item.avgRs?.toFixed(1) ?? ''}</td><td>${item.rs90}</td>` : ''}<td>${item.avgChange?.toFixed(2) ?? ''}</td></tr>`).join(''); const metricHeaders = showRsMetrics ? '<th>平均 RS</th><th>RS 90以上</th>' : ''; const columnCount = showRsMetrics ? 6 : 4; return `<div class="group-wrap"><div class="section-heading"><span>${escapeHtml(label)}｜${state.date} 強勢族群</span></div><div class="table-shell"><table class="group-table"><thead><tr><th>族群</th><th>股票數</th><th>股票清單</th>${metricHeaders}<th>平均漲幅%</th></tr></thead><tbody>${body || `<tr><td colspan="${columnCount}">此股票不在今日強勢族群名單。</td></tr>`}</tbody></table></div></div>`; }
